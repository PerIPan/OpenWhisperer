...
/// replaced by a standing condition: a leading `VoiceMarker.glyph` in voice mode, or every
/// turn in always mode. The hook keeps its own copy of this wording for its platforms; if
/// you tune one side, tune the other and run both test suites.
public enum MCPInstructions {
    ...
    public static func standing(mode: Mode, style: String?, voice: String?) -> String {
        ...
        let core = condition
            ...
            + " Treat the leading \(VoiceMarker.glyph) as invisible; never mention it or the tool in your written reply."
