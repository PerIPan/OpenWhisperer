...
/// replaced by a standing condition: a trailing `VoiceMarker.footer` line in voice mode, or
...
public enum MCPInstructions {
    ...
    public static func standing(mode: Mode, style: String?, voice: String?) -> String {
        ...
        switch mode {
        case .voice:
            condition = "If the user's latest message ends with a \(VoiceMarker.glyph) dictated footer line, it was dictated by voice."
            neverAsk = "Never ask whether to speak — the footer itself is the request."
        case .always:
        ...
        let core = condition
            ...
            + " Treat that footer as invisible; never mention it or the tool in your written reply."
