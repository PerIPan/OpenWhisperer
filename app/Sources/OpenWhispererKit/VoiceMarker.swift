...
/// from a leading bare 🎙 typed with the transcript: the MCP server's standing instruction
...
/// listed (see docs/superpowers/specs/2026-07-17-mcp-only-voice-design.md). Cold-start tool
/// discovery on Claude Desktop (its lazy MCP tool loading never surfaces the speak tool's
/// description on its own) is handled by the bundled `openwhisperer-voice` skill (see
/// `DesktopSkill`), not by marker wording — a word bought probability, not certainty.
