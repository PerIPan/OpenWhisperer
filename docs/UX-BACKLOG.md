...
- **Notch status indicator (overlay split, phase 2).** Move the recording/standby/speaking status out
  of the floating overlay into a small dynamic indicator beside the MacBook notch (Dynamic
  Island-style). Phase 1 — moving transcription history into the menubar dropdown — is specced
  separately (2026-07-13 discussion). Open problems to solve in its own design
  pass: fallback for non-notch displays (fake-notch pill vs. keeping the floating overlay as an
  alternate style); new homes for the overlay's text states (model download
