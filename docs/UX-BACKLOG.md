...
- **Notch status indicator — TRIED AND REJECTED (2026-07-13, PR #24 closed unmerged).** Built,
  installed, smoke-tested live; rejected on the synthesized-lozenge aesthetics on non-notch
  screens, collision risk with a full menu bar, and per-screen duplication. Don't re-propose
  without new facts. The successor direction is the **minimal overlay** (wave + status dot, no
  text) — spec `2026-07-13-minimal-overlay-design.md`. Salvage pointers in the rejected spec.
