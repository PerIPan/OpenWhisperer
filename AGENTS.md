...
**MCP-only tier (Claude Desktop, 2026-07-17).** Claude Desktop has no hooks, so it gets a different mechanism entirely. Dictations targeting it get a leading bare 🎙 glyph plus a trailing trigger line — `Use OpenWhisperer.` — on its own paragraph (`VoiceMarker.glyph`/`trigger`/`apply` in Kit, applied only for allowlisted bundle IDs — `com.anthropic.claudefordesktop`); the MCP server ships a standing instruction (`MCPInstructions` in Kit) via `initialize.instructions` + the speak tool description (guidance *prepended*, not appended — it's the first thing the model reads), regenerated from prefs on every request, keyed on "the latest message begins with 🎙." Deleting both lines silences a turn, typing them force-speaks. The instruction is marker-gated and therefore inert on hook platforms (no `clientInfo` scoping — the transport is stateless). Claude Desktop launches the `--mcp-stdio` bridge from `claude_desktop_config.json`, which proxies to the running app's `:8000/mcp`. Hooks remain the gold path on Claude Code/Codex/agy pending the compliance data in the spec; the marker deliberately does NOT extend to terminal-hosted CLIs (frontmost app ≠ agent focus). The trailing trigger line (not a bundled skill — an earlier design that shipped a `~/.claude/skills` companion skill for cold-start tool discovery was tried and reverted 2026-07-17, vetoed by the product owner as unwilling to write into that shared surface) works because Claude Desktop guarantees the model sees ONLY the user message on a cold chat: tool descriptions load lazily and `initialize.instructions` is discarded, so the trailing line is the one channel guaranteed visible — a terse, direct first-person ask naming the connector that fires Desktop's lazy tool discovery and, live-validated, clears its ask-hedging and injection-wariness gates too (even misspelled, e.g. "OpenWhisper"). Both the standing instruction and the trigger line's first-person framing carry explicit "never ask whether to speak" intent, guarding against a model that loaded the tool but treats speaking as optional. A "first-dictation-per-chat-only" refinement (drop the trigger once a chat's tools are already loaded) is parked in the spec pending a reliable chat-boundary signal.
