...
**Native-tongue flavor.** For a **non-English** voice, `voice-context.sh` reads `tts_voice` and adds two layers to the nudge: **(1) an ungated per-nation persona** — a light standing character (e.g. French = *dry and faintly unimpressed*; Hindi = *irrepressibly helpful*) that colors the reply's *tone, not its vocabulary*, present on **every** non-English voice turn; and **(2) a gated native-word line** on **only ~1 in 5 turns** (a `$RANDOM % 5` gate, pinnable via `OW_FLAVOR_ROLL` for tests) that lets a single actual foreign word slip in, so real code-switching stays occasional and never a tic. First-char map: `f`→French, `i`→Italian, `e`→Spanish, `p`→Brazilian Portuguese, `h`→Hindi, `j`→Japanese, `z`→Mandarin Chinese; English `a`/`b` and unknown get nothing. The map + personas live **only** in the hook — no Swift parity pair (unlike `VoiceSignal.canonicalHash`); `HookTests` guards them (persona sentinel `caricature`, word-layer sentinel `slip in`).
