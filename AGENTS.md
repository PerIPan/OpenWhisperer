...
**MCP-only tier (Claude Desktop, 2026-07-17).** Claude Desktop has no hooks, so it gets a different mechanism entirely. Dictations targeting it get a leading `🎙 speak` (`VoiceMarker.phrase` in Kit — the glyph plus the word "speak", applied only for allowlisted bundle IDs — `com.anthropic.claudefordesktop`); the MCP server ships a standing instruction (`MCPInstructions` in Kit) via `initialize.instructions` + the speak tool description, regenerated from prefs on every request. Deleting the `🎙 speak` silences a turn, typing it force-speaks. The instruction is marker-gated and therefore inert on hook platforms (no `clientInfo` scoping — the transport is stateless). Claude Desktop launches the `--mcp-stdio` bridge from `claude_desktop_config.json`, which proxies to the running app's `:8000/mcp`. Hooks remain the gold path on Claude Code/Codex/agy pending the compliance data in the spec; the marker deliberately does NOT extend to terminal-hosted CLIs (frontmost app ≠ agent focus). The marker's word doubles as the relevance trigger for Desktop's on-demand tool loading (bare 🎙 never loads; "speak" does), so cold chats work without extra setup.
