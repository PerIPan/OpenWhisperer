...
**MCP-only tier (Claude Desktop, 2026-07-17).** Claude Desktop has no hooks, so it gets a different mechanism entirely. Dictations targeting it get a trailing instruction footer — `🎙 dictated — please reply aloud first using the OpenWhisperer speak tool.` — appended on its own paragraph (`VoiceMarker.footer`/`apply` in Kit, applied only for allowlisted bundle IDs — `com.anthropic.claudefordesktop`); the MCP server ships a standing instruction (`MCPInstructions` in Kit) via `initialize.instructions` + the speak tool description (guidance *prepended*, not appended — it's the first thing the model reads), regenerated from prefs on every request, keyed on "the latest message ends with a 🎙 dictated footer line." Deleting the footer silences a turn, typing it force-speaks. The instruction is marker-gated and therefore inert on hook platforms (no `clientInfo` scoping — the transport is stateless). Claude Desktop launches the `--mcp-stdio` bridge from `claude_desktop_config.json`, which proxies to the running app's `:8000/mcp`. Hooks remain the gold path on Claude Code/Codex/agy pending the compliance data in the spec; the marker deliberately does NOT extend to terminal-hosted CLIs (frontmost app ≠ agent focus). The footer (not a leading tag, and not a bundled skill — an earlier design that shipped a `~/.claude/skills` companion skill for cold-start tool discovery was tried and reverted 2026-07-17, vetoed by the product owner as unwilling to write into that shared surface) works because Claude Desktop guarantees the model sees ONLY the user message on a cold chat: tool descriptions load lazily and `initialize.instructions` is discarded, so the footer is the one channel guaranteed visible — it names the connector ("OpenWhisperer") and the `speak` tool (firing Desktop's lazy tool discovery on either anchor) and carries the imperative. Both the standing instruction and the footer wording carry explicit "never ask whether to speak" language, guarding against a model that loaded the tool but treats speaking as optional.
