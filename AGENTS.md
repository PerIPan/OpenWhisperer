...
- WhisperKit → `~/Library/Application Support/OpenWhisperer/models/huggingface/models/argmaxinc/whisperkit-coreml/<model>` (`SpeechTranscriber` explicitly loads with `download: false` when cached). Moved here from the legacy `~/Documents/huggingface` (which is often iCloud-synced, so a 1.5 GB model would upload to iCloud); `ModelStorage.migrateWhisperHubIfNeeded()` renames the old hub into place on launch before the model loads (instant same-volume move), and `ModelStorage.locations` still lists the legacy path so a pre-relocation cache is cleaned up.
