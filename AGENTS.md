...
**MCP-only tier (Claude Desktop, 2026-07-17).** Claude Desktop has no hooks, so it gets a different mechanism entirely. Dictations targeting it get a leading glyph — bare U+1F399 STUDIO MICROPHONE + a space (`VoiceMarker.glyph`/`apply` in Kit, applied only for allowlisted bundle IDs — `com.anthropic.claudefordesktop`) — prepended to the transcript; the MCP server ships a standing instruction (`MCPInstructions` in Kit) via `initialize.instructions` + the speak tool description (guidance *prepended*, not appended — it's the first thing the model reads), regenerated from prefs on every request, keyed on "the latest message begins with 🎙." Deleting the glyph silences a turn, typing it force-speaks. The instruction is marker-gated and therefore inert on hook platforms (no `clientInfo` scoping — the transport is stateless). Claude Desktop launches the `--mcp-stdio` bridge from `claude_desktop_config.json`, which proxies to the running app's `:8000/mcp`. Hooks remain the gold path on Claude Code/Codex/agy pending the compliance data in the spec; the marker deliberately does NOT extend to terminal-hosted CLIs (frontmost app ≠ agent focus).

**Owner-final (2026-07-17), pure glyph, warm-up cost accepted.** After a day of live iteration through worded triggers, a trailing footer, a bundled-skill channel (vetoed by the owner — unwilling to write into the shared `~/.claude/skills` surface), and several closing-line variants (most recently a pure-ASCII `Speak back via OpenWhisperer.` line), the owner chose the original aesthetic back: the bare leading glyph, nothing else. The known cost is documented rather than papered over: wire capture (tee on the stdio bridge) confirmed Desktop's stdio client speaks protocol 2025-11-25 and our `initialize.instructions` goes back intact, but Desktop only surfaces server-level instructions for cloud/account connectors (verified against the owner's own Frankfurter connector), never local stdio servers — so the model may not see the standing instruction on a brand-new chat's first dictated turn, which can land silent. Any message that mentions OpenWhisperer (or asks it to speak) wakes that chat's tool loading; every 🎙 turn after that speaks reliably. Full iteration history and live-probe findings: `docs/superpowers/specs/2026-07-17-mcp-only-voice-design.md`.
