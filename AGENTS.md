...
- **Codex gating:** when the notify payload carries the turn's user messages (`input_messages`, newer Codex builds), `codex-tts-hook.sh` content-correlates them against the `voice_turn` hash — the same mechanism `voice-context.sh` uses for Claude Code — so a foreign turn completing neither steals the signal nor starves the dictated turn. On older payloads without the field it falls back to bare presence+freshness and clears the signal on the first `agent-turn-complete`, where the historical wrong-reply/missed-reply ambiguity still applies. Claude Code's per-session `speak_pending` routing never had this ambiguity.
...
- Version is single-sourced from `Resources/Info.plist` — bump `CFBundleVersion` + `CFBundleShortVersionString` together; `build-dmg.sh` derives the DMG name from `CFBundleShortVersionString` via PlistBuddy.
