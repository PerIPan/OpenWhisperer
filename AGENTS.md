...
**MCP-only tier (Claude Desktop, 2026-07-17).** Claude Desktop has no hooks, so it gets a different mechanism entirely. Dictations targeting it get a trailing marker line — the pure-ASCII imperative `Speak back.` (no glyph — retired after live dark-mode/typing-race findings) — on its own paragraph (`VoiceMarker.marker`/`apply` in Kit, applied only for allowlisted bundle IDs — `com.anthropic.claudefordesktop`); the MCP server ships a standing instruction (`MCPInstructions` in Kit) via `initialize.instructions` + the speak tool description (guidance *prepended*, not appended — it's the first thing the model reads), regenerated from prefs on every request, keyed on "the latest message ends with 'Speak back.'" Deleting the marker silences a turn, typing it force-speaks. The instruction is marker-gated and therefore inert on hook platforms (no `clientInfo` scoping — the transport is stateless). Claude Desktop launches the `--mcp-stdio` bridge from `claude_desktop_config.json`, which proxies to the running app's `:8000/mcp`. Hooks remain the gold path on Claude Code/Codex/agy pending the compliance data in the spec; the marker deliberately does NOT extend to terminal-hosted CLIs (frontmost app ≠ agent focus). The trailing marker line (not a bundled skill — an earlier design that shipped a `~/.claude/skills` companion skill for cold-start tool discovery was tried and reverted 2026-07-17, vetoed by the product owner as unwilling to write into that shared surface) works because Claude Desktop guarantees the model sees ONLY the user message on a cold chat: tool descriptions load lazily and `initialize.instructions` is discarded, so the trailing line is the one channel guaranteed visible. `Speak back.` is the owner-final wording (2026-07-17, superseding the same-day 🎙 signature line): it names the `speak` tool's exact name — the strongest live-measured discovery anchor, firing even on partially-delivered fragments — is imperative (the compliance-validated class), and is two ASCII words, minimizing exposure to the Electron composer's synthetic-typing chunk-reorder race that a surrogate-pair glyph aggravated. Retired for the same reasons: the 🎙 glyph read as an eyesore in dark mode and was functionally unnecessary once the tool-name anchor was found to work alone. A "first-dictation-per-chat-only" refinement (drop the marker once a chat's tools are already loaded) remains parked in the spec pending a reliable chat-boundary signal.
