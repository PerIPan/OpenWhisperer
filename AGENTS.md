...
**MCP-only tier (Claude Desktop, 2026-07-17).** Claude Desktop has no hooks, so it gets a different mechanism entirely. Dictations targeting it get a trailing signature line — `🎙 Sent with OpenWhisperer.` — on its own paragraph (`VoiceMarker.glyph`/`signature`/`apply` in Kit, applied only for allowlisted bundle IDs — `com.anthropic.claudefordesktop`); the MCP server ships a standing instruction (`MCPInstructions` in Kit) via `initialize.instructions` + the speak tool description (guidance *prepended*, not appended — it's the first thing the model reads), regenerated from prefs on every request, keyed on "the latest message ends with the line '🎙 Sent with OpenWhisperer.'" Deleting the signature silences a turn, typing it force-speaks. The instruction is marker-gated and therefore inert on hook platforms (no `clientInfo` scoping — the transport is stateless). Claude Desktop launches the `--mcp-stdio` bridge from `claude_desktop_config.json`, which proxies to the running app's `:8000/mcp`. Hooks remain the gold path on Claude Code/Codex/agy pending the compliance data in the spec; the marker deliberately does NOT extend to terminal-hosted CLIs (frontmost app ≠ agent focus). The trailing signature line (not a bundled skill — an earlier design that shipped a `~/.claude/skills` companion skill for cold-start tool discovery was tried and reverted 2026-07-17, vetoed by the product owner as unwilling to write into that shared surface) works because Claude Desktop guarantees the model sees ONLY the user message on a cold chat: tool descriptions load lazily and `initialize.instructions` is discarded, so the trailing line is the one channel guaranteed visible. The wording is the "Sent from my iPhone" idiom — it pre-reads as an email-style sign-off, so Desktop's injection-wary model has nothing instruction-shaped in the message to refuse, while naming the connector still fires Desktop's lazy tool discovery; the loaded speak-tool description carries the full protocol (speak-first, never-ask) keyed on the signature line. This owner-directed final wording (2026-07-17) supersedes the earlier leading-glyph + "Use OpenWhisperer." trigger line and is declarative rather than imperative — live-validated for tool discovery and ask-hedging/injection-wariness in the prior wording's trials, pending one further live cold-chat probe to confirm the softer signature form still triggers the tool load. A "first-dictation-per-chat-only" refinement (drop the signature once a chat's tools are already loaded) remains parked in the spec pending a reliable chat-boundary signal.
