...
**Native-tongue flavor.** For a **non-English** voice, `voice-context.sh` also reads `tts_voice` and appends a self-gating flavor addendum to the nudge on **only ~1 in 5 voice turns** (a `$RANDOM % 5` gate, pinnable via `OW_FLAVOR_ROLL` for tests, so it stays occasional and never a tic; first-char language map: `f`→French, `i`→Italian, `e`→Spanish, `p`→Brazilian Portuguese, `h`→Hindi, `j`→Japanese, `z`→Mandarin Chinese; English `a`/`b` and unknown get nothing) asking the model to lightly code-switch into the voice's language *unless the spoken sentence is already in it*. The map lives **only** in the hook — there is no Swift parity pair (unlike `VoiceSignal.canonicalHash`); `HookTests` guards it.
